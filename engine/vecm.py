...
def analyze_alpha(result, tickers):
    ...
    for i, ticker in enumerate(tickers):
        ...
        print(f"{ticker:6s}: alpha = {a:8.4f}  ->  {direction}  (~{abs(days_to_revert)} days)")
    return alpha
...
def compute_half_life(alpha):
    ...
    print(f"{'='*50}")
    print(f"Strongest alpha : {strongest_alpha:.4f}")
    ...
    if abs(half_life) < 10:
        ...
        print("Signal          : slow but institutional - tradeable with sizing")
    else:
